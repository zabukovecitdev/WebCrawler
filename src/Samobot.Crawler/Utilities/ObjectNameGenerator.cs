...
namespace Samobot.Crawler.Utilities;
...
public static class ObjectNameGenerator
{
    ...
    public static string GenerateHierarchical(string url, string? contentType = null)
    {
    ...
    public static string GenerateFlat(string url, string? contentType = null)
    {
        if (!Uri.TryCreate(url, UriKind.Absolute, out var uri))
        {
            return GenerateHashBased(url, contentType);
    ...
    {
        var sanitized = InvalidCharsRegex.Replace(url, "_");
        
        if (sanitized.Length > 500)
        {
            var hash = GenerateUrlHash(url);
            sanitized = sanitized[..450] + "_" + hash[..50];
        }
